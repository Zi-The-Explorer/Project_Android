...
class MainActivity : AppCompatActivity() {
    ...
    @Suppress("RedundantSanConstructor")
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        suara_seekbar.setOnSeekBarChangeListener(
        ...
        position_seekbar.max = waktuTotal
        position_seekbar.setOnSeekBarChangeListener(
            object : SeekBar.OnSeekBarChangeListener{
                override fun onProgressChanged(
                    seekBar: SeekBar?,
                    progress: Int,
    ...
    object : Handler() {
        ...
        override fun handleMessage(pesan: Message) {
            ...
            position_seekbar.progress = posisiSaatIni

            val perkiraanWaktu = buatLabelWaktu(posisiSaatIni)
            label_perkiraan_time.text = perkiraanWaktu

            val pengingatWaktu = buatLabelWaktu(waktuTotal - posisiSaatIni)
            label_pengingat_time.text = "-$pengingatWaktu"
        }
    }
    ...
    fun playBtnClick(view: View) {
        if (mediaPutar.isPlaying) {
            mediaPutar.pause()
            btn_play_pause.setBackgroundResource(R.drawable.ic_play_media)
        }else {
            mediaPutar.start()
            btn_play_pause.setBackgroundResource(R.drawable.ic_pause_media)
        }
    }
}