package com.example.linearlayout.activity
...
import com.example.linearlayout.Database
import com.example.linearlayout.EMPModel
import com.example.linearlayout.ItemAdapter
...
class MainActivity : AppCompatActivity() {
    ...
    private fun addRecord(){
        ...
        if (nama.isNotEmpty() && !email.isEmpty() && !phone.isEmpty() && !address.isEmpty()){
            val status = database.addEmployee(EMPModel(0, nama, email, phone, address))
            if (status > -1){
    ...
    private fun getItemsList(): ArrayList<EMPModel>{
        val database: Database = Database(this)
        val empList: ArrayList<EMPModel> = database.viewEmployee()
        return empList
    }
    ...
    fun deleteRecordAlertDialog(empModel: EMPModel){
        ...
        builder.setPositiveButton("Yes") {dialog: DialogInterface, which: Int ->
            val database: Database = Database(this)
            val status = database.deleteEmployee(EMPModel(empModel.id,"", "", empModel.phone, ""))
            if (status > -1){
    ...
    fun updateRecordDialog(empModel: EMPModel){
        ...
        updateDialog.tvUpdate.setOnClickListener {
            ...
            if (!nama.isEmpty() && !email.isEmpty() && !phone.isEmpty() && !address.isEmpty()){
                val status = database.updateEmployee(EMPModel(empModel.id, nama, email, empModel.phone, address))
                if (status > -1){
